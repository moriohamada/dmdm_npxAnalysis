...
from neuron_prediction.glm_ridge.fit import extract_kernels
...
def plot_all_glm_kernels(sess_dir, plots_dir=None, n_workers=1):
    ...
    results_dir = sess_dir / 'glm_ridge_results'

    with open(sess_dir / 'glm_spec.pkl', 'rb') as f:
        col_map = pickle.load(f)

    class_path = sess_dir / 'glm_ridge_classifications.csv'
    class_df = None
    if class_path.exists():
        class_df = pd.read_csv(class_path)

    if plots_dir is None:
        plots_dir = sess_dir / 'glm_ridge_kernels'
    save_dir = Path(plots_dir) / sess.animal / sess.name / 'glm_ridge_kernels'
...
def load_all_classifications(npx_dir=PATHS['npx_dir_local']):
    ...
    for subj in sorted(os.listdir(npx_dir)):
        ...
        for sess_name in sorted(os.listdir(subj_dir)):
            ...
            class_path = os.path.join(sd, 'glm_ridge_classifications.csv')
...
def plot_fraction_significant(npx_dir=PATHS['npx_dir_local'],
    ...
    fig.suptitle('GLM (ridge) lesion significance by region', fontsize=12)
    fig.tight_layout()

    if save_dir:
        save_dir = Path(save_dir) / 'glm_ridge'
        save_dir.mkdir(parents=True, exist_ok=True)
        fig.savefig(save_dir / 'glm_ridge_fraction_significant.png',
